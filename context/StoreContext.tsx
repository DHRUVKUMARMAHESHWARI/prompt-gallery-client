...
interface StoreContextType {
  ...
  signalPromptUsage: (id: string, signal: 'THUMBS_UP' | 'THUMBS_DOWN' | 'NOT_SURE', note?: string) => Promise<void>;
  sessionInteractedPromptIds: string[];
}
...
export const StoreProvider: React.FC<{ children?: ReactNode }> = ({ children }) => {
  ...
  const [sessionInteractedPromptIds, setSessionInteractedPromptIds] = useState<string[]>([]);
  ...
  const signalPromptUsage = async (id: string, signal: 'THUMBS_UP' | 'THUMBS_DOWN' | 'NOT_SURE', note?: string) => {
    // Prevent duplicate signals in session if not intended (though component should handle usage)
    if (!sessionInteractedPromptIds.includes(id)) {
      setSessionInteractedPromptIds(prev => [...prev, id]);
    }

    // Optimistic Update
    setPrompts(prompts.map(p => {
      if (p.id === id) {
        const currentStats = p.usageStats || { thumbsUp: 0, thumbsDown: 0 };
        return {
          ...p,
          usageStats: {
            ...currentStats,
            thumbsUp: signal === 'THUMBS_UP' ? currentStats.thumbsUp + 1 : currentStats.thumbsUp,
            thumbsDown: signal === 'THUMBS_DOWN' ? currentStats.thumbsDown + 1 : currentStats.thumbsDown,
            lastWorked: signal === 'THUMBS_UP' ? Date.now() : currentStats.lastWorked
          }
        };
      }
      return p;
    ...
    if (!isDemoMode) {
      await api.prompts.signalUsage(id, signal, note);
    }
  };
  ...
  return (
    <StoreContext.Provider value={{
      ...
      signalPromptUsage, sessionInteractedPromptIds
    }}>
      {children}
